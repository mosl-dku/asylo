...
#include "asylo/examples/decryptor_server2/decryptor_server_impl.h"
...
N.B: the caller should free the keybytes
				...
				uint8_t *ReadEncKey2(std::string &key_file, int *out_length)
				{
					//	struct stat sbuf;
						uint8_t *p = NULL;
						BIO *inputbio = NULL;
						...
						BIO_write(inputbio, (char *)(key_file.c_str()), 256);
						nrbytes = BIO_read(inputbio, p, 256);
						//BIO_free(inputbio);
...
the ciphering key
				...
				::grpc::Status DecryptorServerImpl::Decrypt(
						...
						std::string enc_key2 = request->key_filename();
						

						//LOG(INFO) << enc_key2;	
												
	
						enc_key = ReadEncKey2(enc_key2,&enc_key_len);
						if (enc_key == NULL) {
								return ::grpc::Status(::grpc::StatusCode::INVALID_ARGUMENT,
